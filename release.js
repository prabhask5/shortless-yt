 ...
 *
 * Bumps the version in package.json, both manifests, and the userscript,
...
const FILES = {
  ...
  userscript: resolve(__dirname, 'userscript/shortless-youtube.user.js'),
};
...
// 3. Update userscript @version
const userscript = readFileSync(FILES.userscript, 'utf-8');
const updatedUserscript = userscript.replace(
  /@version\s+\d+\.\d+\.\d+/,
  `@version      ${newVersion}`
);
writeFileSync(FILES.userscript, updatedUserscript);
