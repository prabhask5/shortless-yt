...
import { getVideoDetails, getComments, getChannelDetails, searchVideos } from '$lib/server/youtube';
...
export const load: PageServerLoad = async ({ url }) => {
	...
	// Build a short search query from the video title (first ~50 chars) for related videos
	const relatedQuery = video.title.slice(0, 50).trim();

	console.log(
		'[WATCH PAGE] Phase 2: Fetching channel details + related videos + comments in parallel'
	);
	const [channels, comments, relatedResult] = await Promise.all([
		...
		searchVideos(relatedQuery, { videoDuration: 'medium' }).catch((err) => {
			console.warn('[WATCH PAGE] Related videos search FAILED:', err);
			return emptyVideos;
		})
	]);
	...
	// Filter related videos: remove the current video, broken videos, and shorts
	const relatedClean = filterOutBrokenVideos(relatedResult.items.filter((v) => v.id !== videoId));
