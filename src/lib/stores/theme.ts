 ...
 *   3. Falls back to `'dark'` on the server (no DOM/media queries available).
 *
 ...
 *          then `'dark'` as the server-side default.
 */
...
export function initTheme(): void {
	...
	// (e.g. the server assumed 'dark' but the user's OS prefers 'light').
	theme.set(t);
}
